...
aqi <- read.csv(
  "C:\\Users\\nandh\\OneDrive\\Desktop\\YuvaIntern_AQI_Analysis\\AQI_2020_to_2025_October.csv",
...
barplot(
  air_quality_count,
  main = "Distribution of Air Quality Categories",
  xlab = "Air Quality Category",
  ylab = "Number of Observations",
  las = 2
)

aqi$Year <- format(aqi$Date, "%Y")
yearly_aqi <- aggregate(
  AQI.Value ~ Year,
  data = aqi,
  FUN = mean
)
...
plot(
  yearly_aqi$Year,
  yearly_aqi$AQI.Value,
  type = "o",
  main = "Average AQI by Year",
  xlab = "Year",
  ylab = "Average AQI"
)
